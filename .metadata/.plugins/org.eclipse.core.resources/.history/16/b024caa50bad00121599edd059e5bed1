...
INLINE event * getfromextraq() {
	//printf("\n queue accessed, extraq length:%d ",disksim->extraq);
	event *temp;
...
INLINE void addtointq(event *newint) {
	...
	case DISKSIM_MASTER:
		if (write(disksim->tracepipes[1], (char *) newint, sizeof(event))
				<= 0) {
	...
	case DISKSIM_SLAVE: {
		...
		ddbg_assert(
				read(disksim->tracepipes[0], (char *)&tmpevt, sizeof(event)) > 0);
...
void scanparam_int(char *parline, char *parname, int *parptr, int parchecks,
		int parminval, int parmaxval) {
	if (sscanf(parline, "%d", parptr) != 1) {
		fprintf(stderr, "Error reading '%s'\n", parname);
		exit(1);
	}
	if (((parchecks & 1) && (*parptr < parminval))
			|| ((parchecks & 2) && (*parptr > parmaxval))) {
		fprintf(stderr, "Invalid value for '%s': %d\n", parname, *parptr);
		exit(1);
	}
}

void getparam_int(FILE *parfile, char *parname, int *parptr, int parchecks,
		int parminval, int parmaxval) {
	char line[201];
...
static void initialize() {
	...
	iotrace_initialize_file(disksim->iotracefile, disksim->traceformat,
	...
	if (curr) {
		...
		if (curr->type == NULL_EVENT) {
			if ((disksim->iotrace) && io_using_external_event(curr)) {
				...
				fprintf(stderr,
						"NULL_EVENT not linked to any external source\n");
				exit(1);
			}
...
//printf ("disksim_checkpoint: simtime %f, totalreqs %d\n", simtime, disksim->totalreqs);
	if (disksim->checkpoint_disable) {
		fprintf(outputfile,
				"Checkpoint at simtime %f skipped because checkpointing is disabled\n",
				simtime);
		return;
	}
	...
	if ((checkpointfile = fopen(checkpointfilename, "w")) == NULL ) {
		fprintf(outputfile,
				"Checkpoint at simtime %f skipped because checkpointfile cannot be opened for write access\n",
				simtime);
		return;
	}
	...
	if (disksim->iotracefile) {
		if (strcmp(disksim->iotracefilename, "stdin") == 0) {
			fprintf(outputfile,
					"Checkpoint at simtime %f skipped because iotrace comes from stdin\n",
					simtime);
			return;
		}
...
void disksim_simulate_event(int num) {
	...
	if ((curr = getnextevent()) == NULL ) {
		...
		if (curr->type == INTR_EVENT) {
		...
		} else if ((curr->type >= IO_MIN_EVENT)
				&& (curr->type <= IO_MAX_EVENT)) {
			io_internal_event((ioreq_event *) curr);
		} else if ((curr->type >= PF_MIN_EVENT)
				&& (curr->type <= PF_MAX_EVENT)) {
			...
			if (disksim->checkpoint_interval) {
				disksim_register_checkpoint(
						simtime + disksim->checkpoint_interval);
			}
...
static void disksim_setup_outputfile(char *filename, char *mode) {
	...
	if (strlen(filename) <= 255) {
		...
		fprintf(stderr,
				"Name of output file is too long (>255 bytes); checkpointing disabled\n");
...
static void disksim_setup_iotracefile(char *filename) {
	if (strcmp(filename, "0") != 0) {
		...
		if (strcmp(filename, "stdin") == 0) {
			...
			if ((disksim->iotracefile = fopen(filename, "rb")) == NULL ) {
				fprintf(stderr,
						"Tracefile %s cannot be opened for read access\n",
						filename);
				exit(1);
			}
		}
	...
	if (strlen(filename) <= 255) {
		...
		fprintf(stderr,
				"Name of iotrace file is too long (>255 bytes); checkpointing disabled\n");
		disksim->checkpoint_disable = 1;
	}
}
...
void disksim_setup_disksim(int argc, char **argv) {
	...
	if (argc < 6) {
		fprintf(stderr,
				"Usage: %s paramfile outfile format iotrace synthgen?\n",
				argv[0]);
		exit(1);
	}
	...
	if (disksim_loadparams(argv[1], disksim->synthgen)) {
		fprintf(stderr,
				"*** error: FATAL: failed to load disksim parameter file.\n");
		exit(1);
	} else {
...
void disksim_restore_from_checkpoint(char *filename) {
	...
	disksim = (disksim_t *) mmap(startaddr, totallength,
	...
	if ((strcmp(disksim->outiosfilename, "0") != 0)
			&& (strcmp(disksim->outiosfilename, "null") != 0)) {
...
void disksim_initialize_vminfo_structure(char *vmconfig_file) {
	...
	FILE *fp;
	fp = fopen(vmconfig_file, "r");
	int index = 0;
	if (fp == NULL ) {
		printf("\n Cannot open VM configuration File %s, exiting ! \n", vmconfig_file);
		exit(1);
	} else {
