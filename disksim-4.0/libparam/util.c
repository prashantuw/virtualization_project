...
char *lp_builtin_names[] = { 0, "Block", "String", "Int", "Double", "List",
		"Topospec" };
...
void lp_unparse_tlts(struct lp_tlt **tlts, int tlts_len, FILE *outfile,
		char *infile) {
	int c;
...
int *lp_instantiate(char *targ, char *name) {
	...
	obj = lp_override_inst(spec, targ, lp_modules[spec->type]->fn, overrides,
			overrides_len);
	...
	for (c = 0; c < b->params_len; c++) {
		if (b->params[c]) {
			if (lp_param_name(b->type, b->params[c]->name) == -1) {
				...
				fprintf(stderr,
						"*** warning: parameter %s not valid in context %s\n",
						b->params[c]->name, lp_modules[b->type]->name);

			}

			else if (lp_modules[b->type]->modvars[lp_param_name(b->type,
					b->params[c]->name)].type != PTYPE(b->params[c])) {
...
struct lp_list *lp_list_add(struct lp_list *l, struct lp_value *v) {
	int c, newlen;
	for (c = 0; c < l->values_len; c++) {
		if (!l->values[c])
			goto done;
...
int lp_param_name(int m, char *n) {
	int c = 0;
	if (!RANGE(m,0,lp_max_mod))
		return -1;
	if (!n)
		return -1;

	while ((c < lp_modules[m]->modvars_len)
			&& strcmp(lp_modules[m]->modvars[c].name, n))
		c++;
...
int lp_mod_name(char *n) {
	...
	while ((c < lp_max_mod) && strcmp(lp_modules[c]->name, n))
		c++;
...
int lp_add_type(char *newt, char *parent) {
	...
	done: lp_typetbl[c] = st;
	return 0;
}

int lp_add_param(struct lp_param ***b, int *plen, struct lp_param *p) {
	int c;
 ...
 * child */
int lp_setup_subtype(struct lp_block *parent, struct lp_block *child) {
	int c, d;
...
static int param_override(struct lp_block *b, char *pname, char *pval) {
	int c;
 ...
 */
int *lp_override_inst(struct lp_block *spec, char *cname, lp_modloader_t loader,
		char **overrides, int overrides_len) {
	int c, d;
		...
		if (range_match(overrides[c], cname)) {
			...
			if (!dumb_split2(overrides[c + 1], &p1, &p2)) {
				...
				for (d = 0; d < spec_copy->params_len; d++) {
					if (!spec_copy->params[d])
						continue;
					if (!strcmp(p1, spec_copy->params[d]->name)) {
						if (spec_copy->params[d]->v->t != BLOCK) {
							fprintf(stderr,
									"*** error: tried to recurse through non-block parameter.\n");
							return 0;
						} else {
				...
				param_override(spec_copy, overrides[c + 1], overrides[c + 2]);
...
int lp_loadfile(FILE *in, struct lp_tlt ***tlts, int *tlts_len, char *infile,
		char **cli_overrides, int cli_overrides_len) {
...
int lp_loadparams(void *it, struct lp_block *b, struct lp_mod *m) {
	...
	for (c = 0; c < b->params_len; c++) {
		...
		TOP: pnum = lp_param_name(lp_mod_name(m->name), b->params[c]->name);
		...
		if (stack_ptr > 0) {
			for (c = 0; c < b->params_len; c++) {
				if (lp_param_name(lp_mod_name(m->name), b->params[c]->name)
						== needed)
					goto FOUND;
			}
			break;
	...
	for (c = 0; c < m->modvars_len; c++) {
		if (m->modvars[c].req && !BIT_TEST(paramvec,c)) {
			fprintf(stderr,
					"*** error: in %s spec -- missing required parameter %s\n",
...
lp_search_path(char *cwd, char *name) {
	...
	for (i = 0; i < lp_searchpath_len; i++) {
		snprintf(cand, LP_PATH_MAX, "%s/%s", lp_searchpath[i], name);
