...
double iodriver_raise_priority(int iodriverno, int opid, int devno, int blkno,
		void *chan) {
...
static int check_send_out_request(iodriver *curriodriver, int devno) {
	...
	if ((curriodriver->consttime == IODRIVER_TRACED_QUEUE_TIMES)
			|| (curriodriver->consttime == IODRIVER_TRACED_BOTH_TIMES)) {
		return (FALSE);
	}
...
static ioreq_event * handle_new_request(iodriver *curriodriver,
		ioreq_event *curr) {
...
static void iodriver_check_c700_based_status(iodriver *curriodriver, int devno,
		int cause, int type, int blkno) {
	ctlr *ctl;
	ioreq_event *tmp;
	...
	if (type == IO_INTERRUPT_ARRIVE) {
		...
		default:
			fprintf(stderr,
					"Unknown interrupt cause at iodriver_check_c700_based_status - cause %d\n",
					cause);
			exit(1);
		}
	} else {
...
static void iodriver_add_to_intrp_eventlist(intr_event *intrp, event *curr,
		double scale) {
	if (curr) {
...
void iodriver_access_complete(int iodriverno, intr_event *intrp) {
	...
	if (iodrivers[iodriverno]->devices[(req->devno)].queuectlr != -1) {
		...
		while (((numreqs) || (tmp != ctl->oversized)) && (tmp) && (tmp->next)
		...
		if ((tmp) && (tmp->next->devno == req->devno)
				&& (tmp->next->opid == req->opid)
				&& (req->blkno >= tmp->next->blkno)
				&& (req->blkno < (tmp->next->blkno + tmp->next->bcount))) {
			fprintf(outputfile,
					"%f, part of oversized request completed: opid %d, blkno %d, bcount %d, maxreqsize %d\n",
					simtime, req->opid, req->blkno, req->bcount,
					ctl->maxreqsize);

			if ((req->blkno + ctl->maxreqsize)
					< (tmp->next->blkno + tmp->next->bcount)) {
	...
	devno = req->devno;
	req = ioqueue_physical_access_done(
			iodrivers[iodriverno]->devices[devno].queue, req);
	...
	if (disksim->traceformat == VALIDATE) {
		...
		if (tmp) {
			...
			disksim_exectrace(
					"Request issue: simtime %f, devno %d, blkno %d, time %f\n",
	...
	while (req) {
		...
		if ((numreqs = logorg_mapcomplete(sysorgs, numsysorgs, tmp)) == COMPLETE) {
			...
			ioreq_event *temp2 = ioqueue_physical_access_done(overallqueue,
					temp);
			ASSERT (temp2 != NULL);
			...
			if (iodrivers[iodriverno]->type != STANDALONE) {
				iodriver_add_to_intrp_eventlist(intrp, io_done_notify(tmp),
			...
			for (i = 0; i < numreqs; i++) {
				...
				if (iodrivers[iodriverno]->type == STANDALONE) {
					...
					iodriver_add_to_intrp_eventlist(intrp, (event *) del,
							iodrivers[iodriverno]->scale);
				}
			}
		}
	...
	if ((iodrivers[iodriverno]->consttime == IODRIVER_TRACED_QUEUE_TIMES)
		...
		if (ioqueue_get_number_in_queue(
				iodrivers[iodriverno]->devices[devno].queue) > 0) {
	...
	schedule_next: if (req) {
		...
		if (iodrivers[iodriverno]->type == STANDALONE) {
			...
			iodriver_add_to_intrp_eventlist(intrp, (event *) req,
					iodrivers[iodriverno]->scale);
		}
	}
}
...
void iodriver_respond_to_device(int iodriverno, intr_event *intrp) {
	...
	iodriver_check_c700_based_status(iodrivers[iodriverno], devno, cause,
			IO_INTERRUPT_COMPLETE, 0);
}
...
static double iodriver_get_time_to_handle_interrupt(iodriver *curriodriver,
		int cause, int read) {
	double retval = 0.0;
	if (cause == DISCONNECT) {
		retval =
		...
		retval = (IODRIVER_BASE_COMPLETION_TIME
				- IODRIVER_COMPLETION_RESPTODEV_TIME) * (double) 5; /* because it gets divided by five later */
...
static double iodriver_get_time_to_respond_to_device(iodriver *curriodriver,
		int cause, double fifthoftotal) {
	double retval;
	if (cause == COMPLETION) {
...
void iodriver_interrupt_arrive(int iodriverno, intr_event *intrp) {
	...
	if ((iodrivers[iodriverno]->consttime == 0.0)
			|| (iodrivers[iodriverno]->consttime == IODRIVER_TRACED_QUEUE_TIMES)) {
		intrp->time = iodriver_get_time_to_handle_interrupt(
				iodrivers[iodriverno], infoptr->cause, (infoptr->flags & READ));
		iodriver_check_c700_based_status(iodrivers[iodriverno], infoptr->devno,
				infoptr->cause, IO_INTERRUPT_ARRIVE, infoptr->blkno);
	} else {
		intrp->time = 0.0;
	}
	if (iodrivers[iodriverno]->type == STANDALONE) {
		...
		tmp = getfromextraq();
		tmp->time = iodriver_get_time_to_respond_to_device(
				iodrivers[iodriverno], infoptr->cause, intrp->time);
...
event * iodriver_request(int iodriverno, ioreq_event *curr) {
	...
	if (outios) {
		fprintf(outios, "%.6f\t%d\t%d\t%d\t%x\n", simtime, curr->devno,
				curr->blkno, curr->bcount, curr->flags);
	}
	...
	disksim->totalreqs++;
	if ((disksim->checkpoint_iocnt > 0)
			&& ((disksim->totalreqs % disksim->checkpoint_iocnt) == 0)) {
	...
	for (; numreqs > 0; numreqs--) {
		...
		curr->next = NULL;
		if ((iodrivers[iodriverno]->consttime == IODRIVER_TRACED_QUEUE_TIMES)
				|| (iodrivers[iodriverno]->consttime
			...
			ret = NULL;
			if ((curr->slotno == 1)
					&& (ioqueue_get_number_in_queue(
							iodrivers[iodriverno]->devices[(curr->devno)].queue)
							== 0)) {
				iodrivers[(iodriverno)]->devices[(curr->devno)].flag = 2;
				iodrivers[(iodriverno)]->devices[(curr->devno)].lastevent =
						simtime;
			}
		}
		ret = handle_new_request(iodrivers[iodriverno], curr);

		if ((ret) && (iodrivers[iodriverno]->type == STANDALONE)
				&& (ret->time == 0.0)) {
...
static void iodriver_set_ctl_to_device(int iodriverno, device * dev) {
	...
	for (i = 0; i < iodrivers[iodriverno]->numctlrs; i++) {
		...
		if ((tmp->slotpath.value & dev->slotpath.value)
				== tmp->slotpath.value) {
			if ((tmp->buspath.value & dev->buspath.value)
					== tmp->buspath.value) {
...
void iodriver_initialize(int standalone) {
	...
	for (i = 0; i < numiodrivers; i++) {
		...
		for (j = 0; j < numdevs; j++) {
			...
			iosim_get_path_to_controller(i, currctlr->ctlno, &currctlr->buspath,
					&currctlr->slotpath);
		...
		curriodriver->devices = (device*) DISKSIM_malloc(
				numdevs * (sizeof(device)));
		...
		for (j = 0; j < numdevs; j++) {
			...
			iosim_get_path_to_device(i, currdev->devno, &currdev->buspath,
					&currdev->slotpath);
			iodriver_set_ctl_to_device(i, currdev);
			get_device_maxoutstanding(curriodriver, currdev);
		}
		logorg_initialize(sysorgs, numsysorgs, queueset, drv_printlocalitystats,
				drv_printblockingstats, drv_printinterferestats,
				drv_printstreakstats, drv_printstampstats, drv_printintarrstats,
				drv_printidlestats, drv_printsizestats);
		/*
...
int load_iodriver_topo(struct lp_topospec *t, int len) {
	...
	if (t->l->values_len < 1) {
		fprintf(stderr,
				"*** error: currently, exactly one bus must be connected to a device driver.\n");
		return 0;
	} else if (t->l->values_len > 1) {
		if (disksim->verbosity > 0)
			fprintf(stderr,
					"*** warning: currently, only one bus is supported per device driver.  I'm ignoring the rest of the buses.\n");
...
int iodriver_load_logorg(struct lp_block *b) {
	...
	newlen =
			disksim->iodriver_info->sysorgs_len ?
...
void iodriver_printstats() {
	...
	if ((drv_printperdiskstats == TRUE)
			&& ((numiodrivers > 1) || (iodrivers[0]->numdevices > 1))) {
		for (i = 0; i < numiodrivers; i++) {
			for (j = 0; j < iodrivers[i]->numdevices; j++) {
