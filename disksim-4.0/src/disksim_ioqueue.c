...
void print_batch_fcfs_queue(subqueue *queue) {
	...
	for (i = 0; i < queue->iobufcnt; i++) {
		printf(" %d %d %d %d %d %d\n", tmp->blkno, tmp->batchno, tmp->state,
				tmp->batch_complete, tmp->next->blkno, tmp->prev->blkno);
...
static void ioqueue_print_subqueue_state(subqueue *queue) {
	...
	fprintf(outputfile,
			"Subqueue state: lastblkno %d, lastcylno %d, lastsurface %d, dir %d\n",
...
int ioqueue_get_number_in_queue(ioqueue *queue) {
	return (queue->base.listlen + queue->timeout.listlen
			+ queue->priority.listlen);
}
...
int ioqueue_get_dist(ioqueue *queue, int blkno) {
	...
	if (queue->lastsubqueue == IOQUEUE_BASE) {
		...
		fprintf(stderr,
				"Unknown queue identification at ioqueue_get_dist - %d\n",
				queue->lastsubqueue);
		exit(1);
	}
...
static void ioqueue_get_cylinder_mapping(ioqueue *queue, iobuf *curr, int blkno,
		int *cylptr, int *surfptr, int cylmaptype) {
	...
	case MAP_ADDSLIPS:
	case MAP_FULL:
		device_get_mapping(cylmaptype, curr->iolist->devno, blkno, cylptr,
				surfptr, NULL );
...
void ioqueue_set_concatok_function(ioqueue *queue,
		int (**concatok)(void *, int, int, int, int), void *concatokparam) {
	queue->concatok = concatok;
	queue->concatokparam = concatokparam;
}

void ioqueue_set_idlework_function(ioqueue *queue,
		void (**idlework)(void *, int), void *idleworkparam, double idledelay) {
...
void ioqueue_set_enablement_function(ioqueue *queue,
...
void ioqueue_reset_idledetecter(ioqueue *queue, int timechange) {
	...
	int listlen = queue->base.listlen + queue->timeout.listlen
			+ queue->priority.listlen;
	...
	if (idledetect) {
		if (!(removefromintq((event *) idledetect))) {
			fprintf(stderr,
					"existing idledetect event not on intq in ioqueue_reset_idledetecter\n");
			exit(1);
		}
	} else {
...
static void ioqueue_remove_from_batch_fcfs_subqueue(subqueue *queue, iobuf *tmp,
		ioreq_event *done) {
	if (queue->sched_alg == TSPS) {
		remove_tsps(tmp);
	}
...
static void ioqueue_insert_batch_fcfs_to_queue(subqueue *queue,
		iobuf *new_iobuf) {
	iobuf *tmp;
...
/* queue->list not NULL when entered */
/* This method takes a request and tries to add into the queue in such a way that
 * the queue retains its property of arranging requests in the order in which they
 * should be executed based on Shortest positioning time.
 *
 * The queue is maintained as a doubly linked list and queue->list points to the last
 * node. When a request has to be taken out for execution from this list, that is done
 * by taking the request which is at the head of the list.
 *
 * Where head is queue->list->next.
 */
static void ioqueue_insert_ordered_to_queue(subqueue *queue, iobuf *temp) {
	...
	/* The conditions are evaluated from right to left */
	if ((temp->cylinder < head->cylinder)
		...
		/* Insert this request at the head of the list i.e just immediately before
		 * existing head but after queue->list which points to the tail of the list.*/
		...
		/* Insert this request at the tail of the list i.e just immediately after
		 * existing tail but before head which points to the first node of the list.*/
		temp->next = head;
		temp->prev = queue->list;
		head->prev = temp;
		queue->list->next = temp;
		...
		/* Find the appropriate position of a node and insert it there. */
		temp->next = run->next;
		temp->prev = run;
		temp->next->prev = temp;
		run->next = temp;
...
static int ioqueue_check_concat(subqueue *queue, iobuf *req1, iobuf *req2,
		int concatmax) {
	ioreq_event *tmp;
	...
	if ((req1 == queue->list) || (req1 == req2) || !READY_TO_GO(req1,queue)
			|| !READY_TO_GO(req2,queue)
			|| ((req1->blkno + req1->totalsize) != req2->blkno)
			|| ((req1->totalsize + req2->totalsize) > concatmax)) {
		return (0);
	}
	if (concatok) {
		if ((*concatok)(concatokparam, req1->blkno, req1->totalsize,
				req2->blkno, req2->totalsize) == 0) {
			return (0);
		}
	}
	tmp = req1->iolist;
...
static int ioqueue_seqstream_head(ioqueue *queue, iobuf *listhead,
		iobuf *current) {
	int idiff;
	int ret = 0;

	if (!(queue->seqscheme
			& (IOQUEUE_SEQSTREAM_READS | IOQUEUE_SEQSTREAM_WRITES
	...
	if (ret == 0) {
		if (current->flags & READ) {
			if (current->prev->flags & READ) {
				if (!(queue->seqscheme
						& (IOQUEUE_SEQSTREAM_READS | IOQUEUE_SEQSTREAM_EITHER))) {
					ret = 1;
				}
			} else {
			...
			if ((current->prev->blkno != current->blkno)
					&& (((current->prev->blkno + current->prev->totalsize
							+ idiff) < current->blkno))) {
				ret = 1;
			}
		} else {
			if (current->prev->flags & READ) {
				...
				if (!(queue->seqscheme
						& (IOQUEUE_SEQSTREAM_WRITES | IOQUEUE_SEQSTREAM_EITHER))) {
					ret = 1;
				}
			}
			if ((current->prev->blkno != current->blkno)
...
static iobuf * ioqueue_get_request_from_pri_lbn_vscan_queue(subqueue *queue,
		int numlbns, int vscan_value) {
	int schedalg;
	int priority_factor;
	int age_factor;
	iobuf *tmp;
	...
	for (i = 0; i < queue->iobufcnt; i++) {
		if (READY_TO_GO(tmp,queue)) {
			...
			curr_effpri += (priority_factor * numlbns
					* ioqueue_get_priority_level(queue->bigqueue, tmp)) / 100;
			curr_effpri += (age_factor * (int) (simtime - tmp->starttime))
					/ 1280;
...
static iobuf * ioqueue_get_request_from_lbn_vscan_queue(subqueue *queue,
		int value) {
	iobuf *temp;
	iobuf *head;
	iobuf *top = NULL;
	...
	if (temp->blkno < queue->lastblkno) {
		...
		if ((bottom == head) && !READY_TO_GO(bottom,queue)) {
			...
			bestbottom = bottom;
			while ((bottom != head)
					&& ((bottom->prev->blkno == bestbottom->blkno)
							|| (!(ioqueue_seqstream_head(queue->bigqueue, head,
									bottom))))) {
				bottom = bottom->prev;
	...
	if ((top == NULL )|| !READY_TO_GO(top,queue) || (!(ioqueue_seqstream_head(queue->bigqueue, head, top)))){temp = bottom;
	...
	if ((top) && READY_TO_GO(top,queue)) {
		...
		ioqueue_get_cylinder_mapping(queue->bigqueue, temp, temp->blkno,
				&cylno1, &surface1, MAP_FULL);
		ioqueue_get_cylinder_mapping(queue->bigqueue, top, top->blkno, &cylno2,
				&surface2, MAP_FULL);
		diff1 = abs(cylno1 - queue->optcylno);
		diff2 = abs(cylno2 - queue->optcylno);
		if (((tmpdir == ASC) && (temp != bottom))
				|| ((tmpdir == DESC) && (temp == bottom))) {
			if (diff2) {
				diff2 += (int) (queue->vscan_value
						* (double) device_get_numcyls(temp->iolist->devno));
			}
		} else if (diff1) {
			if (diff1) {
				diff1 += (int) (queue->vscan_value
						* (double) device_get_numcyls(temp->iolist->devno));
			}
		}
		if (diff1 > diff2) {
...
static iobuf *ioqueue_identify_request_on_cylinder(subqueue *queue) {
	...
	if (lastsurface > temp->surface) {
		...
		while ((temp->next != head) && (lastcylno == temp->next->cylinder)
				&& (!READY_TO_GO(temp,queue) || (temp->blkno < lastblkno))) {
			temp = temp->next;
		}
		...
		while ((temp->next != head) && (lastcylno == temp->next->cylinder)
				&& (lastsurface == temp->next->surface)
				&& (!READY_TO_GO(temp,queue) || (temp->blkno < lastblkno))) {
			temp = temp->next;
		}
		if (READY_TO_GO(temp,queue) && (temp->blkno >= lastblkno)
				&& (temp->surface == lastsurface)) {
			return (temp);
		}
	...
	if (!READY_TO_GO(temp,queue)) {
		while (!READY_TO_GO(temp,queue) && (temp->next != head)
				&& (temp->next->cylinder == lastcylno)) {
			temp = temp->next;
		}
		temp = temp->next;
	}
...
static iobuf * ioqueue_get_request_from_cyl_vscan_queue(subqueue *queue,
		int value) {
	iobuf *temp;
	iobuf *head;
	iobuf *top = NULL;
	...
	if (!READY_TO_GO(bestone,queue)
			|| (bestone->cylinder != queue->lastcylno)) {
		bestone = NULL;
	}
	while ((temp->next != head) && (queue->lastblkno > temp->next->blkno)) {
		temp = temp->next;
	...
	if (temp->blkno < queue->lastblkno) {
		...
		if (READY_TO_GO(bottom,queue) && (!bestone)) {
			while ((bottom != head)
					&& (bottom->prev->cylinder == bottom->cylinder)) {
				bottom = bottom->prev;
			}
	...
	ioqueue_get_cylinder_mapping(queue->bigqueue, temp, temp->blkno, &cylno1,
			&surface1, MAP_FULL);
	diff1 = abs(cylno1 - queue->optcylno);
	if ((top == NULL )|| !READY_TO_GO(top,queue)){
		top = temp;
	...
	ioqueue_get_cylinder_mapping(queue->bigqueue, bottom2, bottom2->blkno,
			&cylno3, &surface3, MAP_FULL);
	...
	for (index = 0; index < VM_IN_USE; index++) {
		if (VM_INFO_ARR[index].vmid == vmid) {
			if (VM_INFO_ARR[index].cur_shares > 0) {
				// The calling method should reduce the cur_shares.
				return true;
			}
			break;
		}
...
//											temp));
		if (READY_TO_GO(temp,queue)
			...
			if (delay < mintime) {
				if (posonly) {
					temp_delay = device_get_servtime(test->devno, test,
							checkcache, (mintime - delay));
					//fprintf(outputfile, "temp_delay = %f\n",temp_delay);
					delay += temp_delay;
				} else {
					delay += device_get_acctime(test->devno, test,
							(mintime - delay));
				}
				if (ageweight == 1) {
...
static double service_time(unsigned int current_cyl,
		unsigned int current_sector, unsigned int current_head,
		unsigned int target_cyl, unsigned int target_sector,
		unsigned int target_head, int read, unsigned int blocks, disk *currdisk) {
	int i;
	double seektime = 0.0;
	double rotation = 0.0;
	double xfer = 0.0;
	...
	double period = dm_time_itod(
			currdisk->model->mech->dm_period(currdisk->model));
	...
	// get the seek time
	{
		...
		nsecs = currdisk->model->mech->dm_seek_time(currdisk->model, &p1, &p2,
				read);
	...
	// get the rotation time
	{
		...
		nsecs = currdisk->model->mech->dm_rottime(currdisk->model, p1.theta,
				p2.theta);
...
static void calc_sp(iobuf **array, subqueue *queue, int checkcache,
		int no_requests) {
	int i;
	int sector;
	ioreq_event *test;
	...
	for (i = 0; i < no_requests; i++) {
		...
		test->flags = temp->flags;
		test->time = simtime;
		device_get_mapping(queue->bigqueue->cylmaptype, temp->iolist->devno,
				temp->blkno, NULL, NULL, &sector);
		if (start == 0) {
			start = 1;
			acc_time = device_get_acctime(test->devno, test, 100000);
		} else {
			acc_time += service_time(last_cylinder, last_sector, last_head,
					temp->cylinder, (unsigned int) sector, temp->surface,
					(temp->flags & READ), temp->totalsize, singledisk);
		}
		last_sector = sector;
...
static void perm(iobuf **array, int m, subqueue *queue, int checkcache,
		int no_requests) {
	int i;
	iobuf *temp;
...
static iobuf *
ioqueue_get_request_from_opt_tsps_queue(subqueue *queue, int checkcache,
		int ageweight, int posonly) {
	int i;
	iobuf *temp;
	iobuf *best = NULL;
	ioreq_event *test;
	...
	if (sched_count != 0) {
		if (requests[current_head] != NULL ) {
			...
			sched_count--;
			fprintf(stderr,
					"2Selected request: %f, cylno %d, blkno %d, read %d, devno %d\n",
					acc_time, best->cylinder, best->blkno, (best->flags & READ),
					best->iolist->devno);
			return (best);
		} else {
	...
	if (sched_count != 0) {
		if (requests[current_head] != NULL ) {
			...
			fprintf(stderr,
					"3Selected request: %f, cylno %d, blkno %d, read %d, devno %d %f\n",
					acc_time, best->cylinder, best->blkno, (best->flags & READ),
					best->iolist->devno, min_time);
			return (best);
		} else {
			fprintf(stderr, "ERROR: request NULL with count != 0\n");
			abort();
...
static iobuf *ioqueue_get_request_from_opt_sptf_rot_weight_queue(
		subqueue *queue, int checkcache, int ageweight, int posonly) {
	int i;
	iobuf *temp;
	ioreq_event *test;
	double readdelay;
		...
		if (READY_TO_GO(temp,queue)
			...
			if (delay < mintime) {
				if (posonly) {
					temp_delay = device_get_servtime(test->devno, test,
							checkcache, (mintime - delay));
					temp_seek = device_get_seektime(test->devno, test,
							checkcache, (mintime - delay));
					delay += temp_delay;
				} else {
					delay += device_get_acctime(test->devno, test,
				...
				if ((delay
						- (temp_delay - temp_seek)
								* queue->bigqueue->latency_weight)
						< (mintime - minsubtract)) {
					best = temp;
					mintime = delay;
					minsubtract = (temp_delay - temp_seek)
							* queue->bigqueue->latency_weight;
				}
			}
		}
...
static iobuf *ioqueue_get_request_from_opt_sptf_weight_queue(subqueue *queue,
		int checkcache, int ageweight, int posonly) {
	int i;
	iobuf *temp;
	ioreq_event *test;
	...
	for (i = 0; i < queue->listlen; i++) {
		if (READY_TO_GO(temp,queue)
			...
			if (delay < mintime) {
				...
				if (delay
						<= (temp_mintime * (1 + queue->bigqueue->latency_weight))) {
...
static iobuf *ioqueue_get_request_from_opt_sptf_seek_weight_queue(
		subqueue *queue, int checkcache, int ageweight, int posonly) {
	int i;
	iobuf *temp;
	iobuf *best = NULL;
...
static iobuf *ioqueue_get_request_from_sdf_queue(subqueue *queue, int exact,
		int direction) {
	...
	for (i = 0; i < queue->listlen; i++) {
		...
		if (READY_TO_GO(temp,queue)
			...
			if (exact) {
				distance = device_get_distance(test->devno, test, -1,
						direction);
			} else {
				distance = device_get_distance(test->devno, test,
...
static iobuf *ioqueue_get_request_from_batch_fcfs_queue(subqueue *queue) {
	...
	printf(
			"ioqueue_get_request_from_batch_fcfs_queue:: none of the requests were READY_TO_GO\n");
	print_batch_fcfs_queue(queue);
...
static ioreq_event * ioqueue_show_next_request_from_subqueue(subqueue *queue) {
	...
	if ((queue->iobufcnt - queue->reqoutstanding) == 0) {
		...
		if (queue->sched_alg == FCFS) {
			...
			temp = ioqueue_get_request_from_pri_lbn_vscan_queue(queue,
					device_get_number_of_blocks(queue->list->devno),
					queue->vscan_cyls);
		} else if (queue->sched_alg == ELEVATOR_LBN) {
			temp = ioqueue_get_request_from_lbn_vscan_queue(queue,
					device_get_number_of_blocks(queue->list->devno));
			...
			temp = ioqueue_get_request_from_cyl_vscan_queue(queue,
					device_get_numcyls(queue->list->devno));
			...
			temp = ioqueue_get_request_from_opt_sptf_rot_weight_queue(queue,
					FALSE, 0, TRUE);
		} else if (queue->sched_alg == SPTF_ROT_WEIGHT) {
			temp = ioqueue_get_request_from_opt_sptf_weight_queue(queue, FALSE,
					0, TRUE);
		} else if (queue->sched_alg == SPTF_SEEK_WEIGHT) {
			temp = ioqueue_get_request_from_opt_sptf_seek_weight_queue(queue,
					FALSE, 0, TRUE);
		} else if (queue->sched_alg == TSPS) {
			temp = ioqueue_get_request_from_opt_tsps_queue(queue, FALSE, 0,
					TRUE);
		} else if (queue->sched_alg == SPTF_OPT) {
			temp = ioqueue_get_request_from_opt_sptf_queue(queue, FALSE, 0,
					TRUE);
			...
			temp = ioqueue_get_request_from_opt_sptf_queue(queue, TRUE, 0,
					TRUE);
			...
			temp = ioqueue_get_request_from_opt_sptf_queue(queue, FALSE, 1,
					TRUE);
		} else if (queue->sched_alg == WPCTF_OPT) {
			temp = ioqueue_get_request_from_opt_sptf_queue(queue, TRUE, 1,
					TRUE);
			...
			temp = ioqueue_get_request_from_opt_sptf_queue(queue, FALSE, 2,
					TRUE);
		} else if (queue->sched_alg == ASPCTF_OPT) {
			temp = ioqueue_get_request_from_opt_sptf_queue(queue, TRUE, 2,
					TRUE);
		} else if (queue->sched_alg == PRI_ASPTF_OPT) {
			temp = ioqueue_get_request_from_opt_sptf_queue(queue, FALSE, 3,
					TRUE);
		} else if (queue->sched_alg == PRI_ASPCTF_OPT) {
			temp = ioqueue_get_request_from_opt_sptf_queue(queue, TRUE, 3,
					TRUE);
	...
	if (temp == NULL ) {
		...
		fprintf(stderr,
				"Selected request is !READY_TO_GO in show_next_request_from_subqueue\n");
		exit(1);
...
static ioreq_event * ioqueue_get_next_request_from_subqueue(subqueue *queue) {
	...
	if ((queue->iobufcnt - queue->reqoutstanding) == 0) {
		...
		if (READY_TO_GO(queue->list,queue)) {
			...
			case VSCAN_CYL:
				if (queue->lastcylno != temp->cylinder) {
					queue->dir =
							(queue->lastcylno < temp->cylinder) ? ASC : DESC;
				}
				break;
				/*
				 case BATCH_FCFS:
	...
	if (temp == NULL ) {
		...
		fprintf(stderr,
				"Selected request is !READY_TO_GO in get_next_request_from_subqueue\n");
		exit(1);
	}
	/*
	...
	case SSTF_LBN:
		if (queue->sched_alg != VSCAN_CYL) {
			...
			if (queue->lastblkno
					!= device_get_number_of_blocks(temp->iolist->devno)) {
				break;
			}
		}
	case BATCH_FCFS:
	...
	ioqueue_get_cylinder_mapping(queue->bigqueue, temp, queue->lastblkno,
			&queue->lastcylno, &queue->lastsurface, -1);
	ioqueue_get_cylinder_mapping(queue->bigqueue, temp, queue->lastblkno,
			&queue->optcylno, &queue->optsurface, MAP_FULL);

	if (temp->starttime == -1.0) {
		temp->starttime = simtime;
	}
	ret = temp->iolist;
	stat_update(&queue->instqueuelen,
			(double) (queue->iobufcnt - queue->reqoutstanding));
	...
	if (temp->iolist == NULL ) {
		fprintf(stderr,
				"iobuf structure with no iolist chosen to be scheduled\n");
		exit(1);
...
static int ioqueue_request_match(ioreq_event *wanted, iobuf *curr) {
	...
	if (curr->batch_list != NULL ) {
		...
		if ((curr->blkno <= wanted->blkno)
				&& ((curr->blkno + curr->totalsize) >= wanted->blkno)) {
			...
			tmp = curr->iolist;
			while (tmp) {
				if ((tmp->blkno == wanted->blkno)
						&& (tmp->opid == wanted->opid)) {
					return (1);
				}
				tmp = tmp->next;
			}
...
static ioreq_event * ioqueue_get_specific_request_from_subqueue(subqueue *queue,
		ioreq_event *wanted) {
	iobuf *temp;
	ioreq_event *ret;
	...
	if ((queue->iobufcnt - queue->reqoutstanding) == 0) {
		...
		temp = queue->list;
		while ((temp->next != queue->list)
				&& (ioqueue_request_match(wanted, temp) == 0)) {
			temp = temp->next;
		}
	}
...
static ioreq_event * ioqueue_set_starttime_in_subqueue(subqueue *queue,
		ioreq_event *target) {
	...
	if ((queue->iobufcnt - queue->reqoutstanding) == 0) {
		...
		while ((temp->next != queue->list)
				&& (!ioqueue_request_match(target, temp))) {
			temp = temp->next;
		}
	}
...
static ioreq_event * ioqueue_remove_completed_request(subqueue *queue,
		ioreq_event *done) {
	iobuf *tmp;
	iobuf *tail;
	ioreq_event *trv;
	...
	if ((tmp->state != PENDING) || (ioqueue_request_match(done, tmp) == 0)) {
		...
		if ((tmp->state != PENDING)
				|| (ioqueue_request_match(done, tmp) == 0)) {
			fprintf(stderr,
					"Completed event not found pending in ioqueue - blkno %d, tmp->blkno %d, tmp->totalsize %d, %d, %d, %d, %d\n",
					done->blkno, tmp->blkno, tmp->totalsize,
					(tmp == NULL ), done->opid, tmp->opid, done->bcount);
	...
	if (tmp->reqcnt == 1) {
		...
		if (tmp->flags & READ) {
			if (tmp->flags & TIME_CRITICAL) {
				stat_update(&queue->critreadstats,
						(simtime - tmp->iob_un.time));
			} else {
				stat_update(&queue->nocritreadstats,
						(simtime - tmp->iob_un.time));
			}
		} else {
			if (tmp->flags & TIME_CRITICAL) {
				stat_update(&queue->critwritestats,
						(simtime - tmp->iob_un.time));
			} else {
				stat_update(&queue->nocritwritestats,
						(simtime - tmp->iob_un.time));
			}
		}
	} else {
			...
			while (batch_ptr != NULL ) {
				if ((batch_ptr->blkno == done->blkno)
					...
					if (tmp->batch_size == 1) {
						...
						batch_ptr->batch_prev->batch_next =
								batch_ptr->batch_next;
						if (batch_ptr->batch_next != NULL ) {
							batch_ptr->batch_next->batch_prev =
...
ioreq_event * ioqueue_get_next_request(ioqueue *queue) {
	...
	//printf("\n base queue max length %d %d %d %d %d",queue->maxqlen,queue->maxlistlen,queue->maxoutstanding,queue->maxreadlen,queue->maxwritelen );
	/*
	...
	if (tmp) {
		int numout = queue->base.numoutstanding + queue->timeout.numoutstanding
				+ queue->priority.numoutstanding;
...
double ioqueue_add_new_request(ioqueue *queue, ioreq_event *new) {
	...
	case ALLEQUAL:
		if ((queue->base.sched_alg == ELEVATOR_LBN)
			...
			ioqueue_get_cylinder_mapping(queue, tmp, tmp->blkno, &tmp->cylinder,
					&tmp->surface, MAP_NONE);
		} else {
			ioqueue_get_cylinder_mapping(queue, tmp, tmp->blkno, &tmp->cylinder,
					&tmp->surface, -1);
		}
	...
	case TWOQUEUE:
		if (new->flags & (TIME_CRITICAL | TIME_LIMITED)) {
			if ((queue->priority.sched_alg == ELEVATOR_LBN)
				...
				ioqueue_get_cylinder_mapping(queue, tmp, tmp->blkno,
						&tmp->cylinder, &tmp->surface, MAP_NONE);
			} else {
				ioqueue_get_cylinder_mapping(queue, tmp, tmp->blkno,
						&tmp->cylinder, &tmp->surface, -1);
			}
	...
	default:
		fprintf(stderr,
				"Unknown priority scheme being employed at ioqueue_add_new_request\n");
		exit(1);
	}
	listlen = queue->base.listlen + queue->timeout.listlen
			+ queue->priority.listlen;
	qlen = listlen
			- (queue->base.numoutstanding + queue->timeout.numoutstanding
					+ queue->priority.numoutstanding);
	readlen = queue->base.readlen + queue->timeout.readlen
			+ queue->priority.readlen;
	...
	if (queue->idledetect) {
		if (!(removefromintq((event *) queue->idledetect))) {
			fprintf(stderr,
					"existing idledetect event not on intq in ioqueue_add_new_request\n");
			exit(1);
		}
...
static int ioqueue_raise_priority_subqueue(subqueue *queue, subqueue *priority,
		int opid, int pri_scheme) {
	int stop = FALSE;
	iobuf *tmp;
	ioreq_event *trv;
	iobuf *bump;
...
int ioqueue_raise_priority(ioqueue *queue, int opid) {
	...
	found = ioqueue_raise_priority_subqueue(&queue->base, &queue->priority,
			opid, queue->pri_scheme);
	if (found) {
		return (found);
	}
	found = ioqueue_raise_priority_subqueue(&queue->timeout, &queue->priority,
			opid, queue->pri_scheme);
	return (found);
}

static int ioqueue_implement_timeouts(subqueue *queue, subqueue *timeout,
		int maxtime, u_int flag) {
	iobuf *tmp;
	iobuf *done;
	ioreq_event *trv;
	int timeouts = 0;
...
double ioqueue_tick(ioqueue *queue) {
	...
	if (queue->to_scheme == BASETIMEOUT) {
		if (queue->pri_scheme == ALLEQUAL) {
			...
			timeouts = ioqueue_implement_timeouts(&queue->base, &queue->timeout,
					queue->to_time, TIMED_OUT);
			...
			timeouts = ioqueue_implement_timeouts(&queue->base,
					&queue->priority, queue->to_time, TIMED_OUT);
			queue->timeouts += timeouts;
		 ...
		 */
		readcnt = queue->priority.numreads;
		timeouts = ioqueue_implement_timeouts(&queue->timeout, &queue->priority,
				queue->to_time, TIMED_OUT);
		queue->timeouts += timeouts;
		queue->timeoutreads += queue->priority.numreads - readcnt;
		/*
...
static void ioqueue_clobber_overlaps_subqueue(subqueue *queue, ioreq_event *ret,
		double arrtimemax) {
	iobuf *tmp;
	ioreq_event *done;
	int read = ret->flags & READ;
	while ((tmp = queue->list)) {
		if ((tmp->state != WAITING) || (tmp->blkno != ret->blkno)
				|| (tmp->totalsize != ret->bcount)
				|| ((read) && !(tmp->flags & READ))
				|| (tmp->iolist->time > arrtimemax)) {
			tmp = tmp->next;
			while ((tmp != queue->list)
					&& ((tmp->state != WAITING) || (tmp->blkno != ret->blkno)
							|| (tmp->totalsize != ret->bcount)
							|| ((read) && !(tmp->flags & READ))
							|| (tmp->iolist->time > arrtimemax))) {
				tmp = tmp->next;
...
ioreq_event * ioqueue_physical_access_done(ioqueue *queue, ioreq_event *curr) {
	...
	queue->idlestart = simtime;
	if ((queue->pri_scheme != ALLEQUAL)
			&& ((curr->flags & (TIME_CRITICAL | TIME_LIMITED))
					|| ((queue->to_scheme != NOTIMEOUT)
							&& (curr->flags & TIMED_OUT)))) {
		/*
		 fprintf(outputfile, "Checking the priority queue\n");
		 */
		ret = ioqueue_remove_completed_request(&queue->priority, curr);
	} else if ((queue->to_scheme != NOTIMEOUT)
			&& (curr->flags & (TIMED_OUT | HALF_OUT))) {
		/*
...
static void ioqueue_subqueue_initialize(subqueue *queue, int devno) {
	...
	stat_initialize(statdeffile, statdesc_outtimestats,
			&queue->nocritreadstats);
	stat_initialize(statdeffile, statdesc_outtimestats, &queue->critwritestats);
	stat_initialize(statdeffile, statdesc_outtimestats,
			&queue->nocritwritestats);
	...
	if ((queue->sched_alg == VSCAN_LBN)
			|| (queue->sched_alg == PRI_VSCAN_LBN)) {
...
void ioqueue_initialize(ioqueue *queue, int devno) {
	...
	stat_initialize(statdeffile, statdesc_readintarrstats,
			&queue->readintarrstats);
	stat_initialize(statdeffile, statdesc_writeintarrstats,
			&queue->writeintarrstats);
	...
	stat_initialize(statdeffile, statdesc_writesizestats,
			&queue->writesizestats);
	stat_initialize(statdeffile, statdesc_batchsizestats,
			&queue->batchsizestats);
}
...
void ioqueue_cleanstats(ioqueue *queue) {
	...
	if ((queue->base.listlen + queue->timeout.listlen + queue->priority.listlen)
			== 0) {
...
static void ioqueue_printqueuestats(ioqueue **set, int setsize, char *prefix) {
	...
	for (i = 0; i < setsize; i++) {
		...
		numcomplete += set[i]->base.numcomplete + set[i]->timeout.numcomplete
				+ set[i]->priority.numcomplete;
		runlistlen += set[i]->base.runlistlen + set[i]->timeout.runlistlen
				+ set[i]->priority.runlistlen;
		runoutstanding += set[i]->base.runoutstanding
				+ set[i]->timeout.runoutstanding
				+ set[i]->priority.runoutstanding;
		listlen += set[i]->base.listlen + set[i]->timeout.listlen
				+ set[i]->priority.listlen;
		numoutstanding += set[i]->base.numoutstanding
				+ set[i]->timeout.numoutstanding
				+ set[i]->priority.numoutstanding;
		runreadlen += set[i]->base.runreadlen + set[i]->timeout.runreadlen
				+ set[i]->priority.runreadlen;
	}
	...
	fprintf(outputfile, "%sAvg # read requests:     %f\n", prefix,
			(runreadlen / ((simtime - warmuptime)* (double) setsize)));
	fprintf(outputfile, "%sMax # read requests:     %d\n", prefix, maxreadlen);
	fprintf(outputfile, "%sAvg # write requests:    %f\n", prefix,
			(runwritelen / ((simtime - warmuptime)* (double) setsize)));
	...
	fprintf(outputfile, "%sNumber of batches:  %d\n", prefix,
			stat_get_count_set(statset, setsize));
...
static void ioqueue_printcritstats(subqueue **set, int setsize, char *prefix,
		int numreqs) {
	char prefix2[256];
	int cnt;
	int i;
	...
	fprintf(outputfile, "%sCritical Reads:      \t%6d  \t%f\n", prefix, cnt,
			((double) cnt / (double) max(numreqs,1)));
...
static void ioqueue_subqueue_printstats(subqueue **set, int setsize,
		char *prefix, int printcritstats) {
	int numcomplete = 0;
	int maxlistlen = 0;
	int listlen = 0;
	int maxqlen = 0;
...
void ioqueue_printstats(ioqueue **set, int setsize, char *sourcestr) {
	...
	for (i = 0; i < setsize; i++) {
		...
		if ((set[i]->to_scheme != NOTIMEOUT)
				|| (set[i]->pri_scheme != ALLEQUAL)) {
		...
		numreads += set[i]->base.numreads + set[i]->timeout.numreads
				+ set[i]->priority.numreads;
		/*
		 numreads -= (set[i]->timeoutreads + set[i]->halfoutreads);
		 */
		numwrites += set[i]->base.numwrites + set[i]->timeout.numwrites
				+ set[i]->priority.numwrites;
		/*
		...
		timeout_num_sptf_sdf_different +=
				set[i]->timeout.num_sptf_sdf_different;
		timeout_num_scheduling_decisions +=
				set[i]->timeout.num_scheduling_decisions;
		priority_num_sptf_sdf_different +=
				set[i]->priority.num_sptf_sdf_different;
		priority_num_scheduling_decisions +=
				set[i]->priority.num_scheduling_decisions;
	...
	fprintf(outputfile, "%sRequests per second:   \t%f\n", sourcestr,
			((double) 1000 * (double) numcomplete / (simtime - warmuptime)));
	fprintf(outputfile, "%sCompletely idle time:  \t%f   \t%f\n", sourcestr,
			idletime, (idletime / (simtime - warmuptime)));

	stat_print_set(statset, (3 * setsize), sourcestr);

	fprintf(outputfile, "%sOverlaps combined:     \t%d\t%f\n", sourcestr,
			overlapscombed,
			((double) overlapscombed / (double) max(numcomplete,1)));
	fprintf(outputfile, "%sRead overlaps combined:\t%d\t%f\t%f\n", sourcestr,
			readoverlapscombed,
			((double) readoverlapscombed / (double) max(overlapscombed,1)), ((double) readoverlapscombed / (double) max(numreads,1)));
	...
	fprintf(outputfile, "%sNumber of reads:    %6d  \t%f\n", sourcestr,
			numreads, ((double) numreads / max(numreqs,1)));
	fprintf(outputfile, "%sNumber of writes:   %6d  \t%f\n", sourcestr,
			numwrites, ((double) numwrites / max(numreqs,1)));
	fprintf(outputfile, "%sSequential reads:   %6d  \t%f  \t%f\n", sourcestr,
			seqreads,
			((double) seqreads / (double) max(numreads,1)), ((double) seqreads / max(numreqs,1)));
	fprintf(outputfile, "%sSequential writes:  %6d  \t%f  \t%f\n", sourcestr,
			seqwrites,
			((double) seqwrites / (double) max(numwrites,1)), ((double) seqwrites / max(numreqs,1)));

	fprintf(outputfile, "%sBase SPTF/SDF Different: %6d / %6d \t%f\n",
			sourcestr, base_num_sptf_sdf_different,
			base_num_scheduling_decisions,
			((double) base_num_sptf_sdf_different
					/ (double) max(base_num_scheduling_decisions, 1)));
	fprintf(outputfile, "%sTimeout SPTF/SDF Different: %6d / %6d \t%f\n",
			sourcestr, timeout_num_sptf_sdf_different,
			timeout_num_scheduling_decisions,
			((double) timeout_num_sptf_sdf_different
					/ (double) max(timeout_num_scheduling_decisions, 1)));
	fprintf(outputfile, "%sPriority SPTF/SDF Different: %6d / %6d \t%f\n",
			sourcestr, priority_num_sptf_sdf_different,
			priority_num_scheduling_decisions,
			((double) priority_num_sptf_sdf_different
					/ (double) max(priority_num_scheduling_decisions, 1)));
	...
	if (subcheck) {
		...
		if (to_scheme != NOTIMEOUT) {
			...
			fprintf(outputfile, "%sHalfway timed out requests:           %d\n",
					sourcestr, halfouts);
			ioqueue_subqueue_printstats(subset, setsize, prefix,
					printcritstats);
		}
