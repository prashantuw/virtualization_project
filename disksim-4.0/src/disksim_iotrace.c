...
void iotrace_set_format (char *formatname)
{
   ...
   } else if (strcmp(formatname, "vmm") == 0) {
		/* VMM trace file */
	      disksim->traceformat = VMM;
...
static ioreq_event * iotrace_vmm_get_ioreq_event (FILE *tracefile, ioreq_event *new)
{
   char line[201];
   ...
   if (sscanf(line, "%d %lf %d %d %d %x\n",&new->vmid,&new->time, &new->devno, &new->blkno, &new->bcount, &new->flags) != 6) {
      fprintf(stderr, "Wrong number of arguments for I/O trace event type\n");
      fprintf(stderr, "line: %s", line);
      ddbg_assert(0);
   }
...
ioreq_event * iotrace_get_ioreq_event (FILE *tracefile, int traceformat, ioreq_event *temp)
{
   ...
   case VMM:
         temp = iotrace_vmm_get_ioreq_event(tracefile, temp);
      break;
